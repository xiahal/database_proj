...
namespace database_project
{
    public partial class teacherScoreInput : System.Windows.Forms.Form
    {
        ...
        public teacherScoreInput(string id)
        {
            ...
            tid = id;
            int rows = 0;
            string sql_getcid = "select cid from choices,teachers where teachers.tid=choices.tid and teachers.tid = '" + tid + "'";
            SqlConnection con_getcid = new SqlConnection(connectionString);    //创建一个数据库连接
        ...
        private void confirmInput_Click(object sender, EventArgs e)
        {
            ...
            {
                string sql_upd = "update choices set cscore = " + ipt_score + " where tid = '" + tid + "' and sid = '" + ipt_sid + "' and cid = '" + ipt_cid + "'";
                ...
                try
                {
                    ...
                    {
                        MessageBox.Show("choices RECORD does NOT exists!");
                        classIdBox.Text = "";
                        studentIdBox.Text = "";
                        studentScoreBox.Text = "";
                    }
